...
const ENVIRONMENTS: EnvironmentConfig[] = [
  {
    id: "none",
    name: "No Background",
    icon: <LuVolumeX className="w-5 h-5" />,
  ...
  },
  {
    ...
    description: "Coffee shop chatter, espresso machines",
    audioUrl: "/audio/ambiance/cafe.mp3",
  },
  {
    id: "office",
    name: "Office",
    icon: <LuBuilding2 className="w-5 h-5" />,
    description: "Keyboard typing, printer, quiet murmurs",
    audioUrl: "/audio/ambiance/office.mp3",
  },
  {
    id: "restaurant",
    name: "Restaurant",
    icon: <LuUtensils className="w-5 h-5" />,
    description: "Dishes clinking, conversations, kitchen sounds",
    audioUrl: "/audio/ambiance/restaurant.mp3",
  },
  {
    id: "hospital",
    name: "Hospital",
    icon: <LuStethoscope className="w-5 h-5" />,
    description: "Quiet hallways, distant announcements",
    audioUrl: "/audio/ambiance/hospital.mp3",
  },
  {
    id: "street",
    name: "City Street",
    icon: <LuCar className="w-5 h-5" />,
    description: "Traffic, pedestrians, city sounds",
    audioUrl: "/audio/ambiance/street.mp3",
  },
  {
    id: "train_station",
    name: "Train Station",
    icon: <LuTrain className="w-5 h-5" />,
    description: "Announcements, train sounds, crowd",
    audioUrl: "/audio/ambiance/train.mp3",
  },
  {
    id: "airport",
    name: "Airport",
    icon: <LuPlane className="w-5 h-5" />,
    description: "Announcements, crowds, rolling luggage",
    audioUrl: "/audio/ambiance/airport.mp3",
  },
  {
    id: "park",
    name: "Park",
    icon: <LuTrees className="w-5 h-5" />,
    description: "Birds singing, wind, nature sounds",
    audioUrl: "/audio/ambiance/park.mp3",
  },
  {
    id: "home",
    name: "Home",
    icon: <LuHouse className="w-5 h-5" />,
    description: "Quiet indoor ambiance",
    audioUrl: "/audio/ambiance/home.mp3",
  },
  {
    id: "supermarket",
    name: "Supermarket",
    icon: <LuShoppingCart className="w-5 h-5" />,
    description: "Shopping carts, beeping, announcements",
    audioUrl: "/audio/ambiance/supermarket.mp3",
  },
  {
    id: "library",
    name: "Library",
    icon: <LuLibrary className="w-5 h-5" />,
    description: "Very quiet, page turns, whispers",
    audioUrl: "/audio/ambiance/library.mp3",
  },
];
...
const VOICE_OPTIONS = [
  {
    ...
    description: "Rachel - Clear and professional",
  },
  {
    id: "male_professional",
    name: "Male (Professional)",
    description: "Adam - Deep and authoritative",
  },
  {
    id: "male_casual",
    name: "Male (Casual)",
    description: "Antoni - Warm and conversational",
  },
  {
    id: "female_professional",
    name: "Female (Professional)",
    description: "Bella - Confident and articulate",
  },
];
...
function ConversationContent() {
  ...
  const [customEnvironment, setCustomEnvironment] =
    useState<EnvironmentType>("none");
  ...
  const elevenLabsConversation = useConversation({
    onConnect: () => {
      console.log(
        "[ElevenLabs] onConnect callback fired! Status:",
    ...
    onMessage: (message) => {
      console.log("[ElevenLabs] onMessage callback fired:", message);
      if (message.message) {
    ...
    onError: (error) => {
      console.error("[ElevenLabs] onError callback fired:", error);
      setIsLoading(false);
    },
  });
  ...
  useEffect(() => {
    ...
    if (sessionIdFromUrl) {
      ...
      fetch(`/api/conversation/sessions/${sessionIdFromUrl}`)
        .then((res) =>
          res.ok ? res.json() : Promise.reject("Session not found"),
        )
        .then((data) => {
          ...
          setSelectedEnvironment(
            (session.environment || "none") as EnvironmentType,
          );
  ...
  useEffect(() => {
    ...
    if (view === "chat" && selectedEnvironment !== "none") {
      ...
      if (env?.audioUrl && audio) {
        audio.src = env.audioUrl;
  ...
  const buildFirstMessage = useCallback((scenario: Scenario): string => {
    ...
    if (
      scenarioLower.includes("landlord") ||
      scenarioLower.includes("apartment")
    ) {
      return "Hallo! Schön, dass Sie sich für die Wohnung interessieren. Kommen Sie herein!";
    }
    if (
      scenarioLower.includes("work") ||
      scenarioLower.includes("office") ||
      scenarioLower.includes("colleague")
    ) {
  ...
  const startConversation = useCallback(
    async (scenario: Scenario) => {
      ...
      setMessages([]);
      const environment =
        scenario.defaultEnvironment || customEnvironment || "none";
      ...
      if (inputMode === "elevenlabs") {
        ...
        try {
          ...
          // The signedUrl is a wss:// WebSocket URL that contains authentication info
          console.log("[ElevenLabs] Starting session with signed URL...");
          console.log("[ElevenLabs] Status before startSession:", elevenLabsConversation.status);
          ...
          try {
            ...
            // Start session with signedUrl and connectionType
            await elevenLabsConversation.startSession({
              signedUrl: signedUrl,
              connectionType: "websocket",
            });
            
            console.log("[ElevenLabs] startSession initiated successfully");
          } catch (sessionError) {
            console.error("[ElevenLabs] Error calling startSession:", sessionError);
            throw sessionError;
          }
          ...
          setTimeout(() => {
            console.log("[ElevenLabs] Connection check - current status:", elevenLabsConversation.status);
            if (elevenLabsConversation.status !== "connected") {
              console.error("[ElevenLabs] Connection timeout - status:", elevenLabsConversation.status);
              console.log("[ElevenLabs] Falling back to text mode due to timeout");
              setInputMode("text");
              setIsLoading(false);
            }
          }, 10000); // 10 second timeout
          // Note: setIsLoading(false) is called in onConnect callback when connection is established
        } catch (error) {
          ...
          try {
            const response = await fetch("/api/conversation", {
              ...
              body: JSON.stringify({
                ...
                isStart: true,
                customDescription:
                  scenario.id === "custom" ? customScenario : undefined,
                environment: environment,
              }),
            });
            if (response.ok) {
  ...
  if (showContinueDialog && savedConversation) {
    const timeAgo = Math.round(
      (Date.now() - savedConversation.timestamp) / 60000,
    );
    const timeText =
      timeAgo < 60
      ...
      <div className="max-w-md mx-auto px-4 py-12">
        <div className="bg-card border border-border rounded-2xl p-8 text-center">
          <span className="flex justify-center mb-4">
            <LuMessageCircle className="w-12 h-12 text-primary" />
          </span>
          ...
          <div className="bg-muted/50 rounded-xl p-4 mb-6 text-left">
            <div className="text-sm font-medium mb-1">
              {savedConversation.scenario.name}
            </div>
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <p className="text-muted-foreground mb-8">
          Practice speaking in realistic scenarios with AI-powered voice
          conversations!
        </p>
        ...
        >
          <span className="flex justify-center mb-2">
            <LuSparkles className="w-7 h-7 text-primary" />
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {scenarios.map((scenario) => {
            const defaultEnv = ENVIRONMENTS.find(
              (e) => e.id === scenario.defaultEnvironment,
            );
            return (
              <button
                key={scenario.id}
              ...
              >
                <div className="flex items-start justify-between mb-1">
                  ...
                  {defaultEnv && (
                    <span
                      className="text-lg"
                      title={`Environment: ${defaultEnv.name}`}
                    >
                      {defaultEnv.icon}
                    </span>
                  )}
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <span className="flex justify-center mb-4">
            <FaTheaterMasks className="w-14 h-14 text-primary" />
          </span>
          ...
          <div className="mb-6">
            ...
            <ul className="space-y-2">
              {feedback.strengths.map((strength, i) => (
                <li
                  key={i}
                  className="text-sm text-muted-foreground pl-4 border-l-2 border-green-500"
                >
                  {strength}
                </li>
              ))}
        ...
        {/* New Words Learned */}
        {feedback.newWordsLearned && feedback.newWordsLearned.length > 0 && (
          <div className="mb-6">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <LuBookOpen className="w-5 h-5 text-green-500" /> New Words Added to Memory
            </h3>
            <div className="flex flex-wrap gap-2">
              {feedback.newWordsLearned.map((word, i) => (
                <span
                  key={i}
                  className="px-3 py-1.5 rounded-full bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20 text-sm font-medium"
                >
                  {word}
                </span>
              ))}
            ...
            <p className="text-sm text-muted-foreground mt-3">
              These words have been saved to your memory and will appear in review sessions.
            </p>
          </div>
        )}
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-4 h-[calc(100dvh-8rem)] flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4 pb-4 border-b border-border">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {currentEnv && currentEnv.id !== "none" && (
              <span
                className="text-lg"
                title={`Environment: ${currentEnv.name}`}
              >
                {currentEnv.icon}
              </span>
            )}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {inputMode === "elevenlabs" ? (
              <>
                <LuMic className="w-3 h-3" /> Voice
              </>
            ) : (
              <>
                <LuKeyboard className="w-3 h-3" /> Text
              </>
            )}
          </span>
          <button
      ...
      {/* ElevenLabs Status */}
      {inputMode === "elevenlabs" && (
        <div className="mb-4 p-3 rounded-xl bg-linear-to-r from-green-500/10 to-blue-500/10 border border-green-500/20">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium flex items-center gap-1">
                {elevenLabsConversation.status === "connected" ? (
                  elevenLabsConversation.isSpeaking ? (
                    <>
                      <LuVolume2 className="w-4 h-4" /> AI Speaking...
                    </>
                  ) : (
                    <>
                      <IoEarOutline className="w-4 h-4" /> Listening...
                    </>
                  )
                ) : (
                  "Connecting..."
